...
- [ ] **Volume gate no auditor pré-publicação**: o auditor IA deve chumbar ou avisar formalmente quando o plano fica abaixo de MEV em ≥2 grupos musculares ou acima de MRV em qualquer grupo. Hoje o `VolumeSection` mostra esses semáforos, mas só como leitura — o PT pode publicar à mesma. Trabalho: validador determinístico após stage 5, com bloqueio opcional para tier Studio.
- [ ] **Volume realizado vs prescrito na Tab Resultados**: hoje só temos "tonelagem realizada" agregada. Falta a comparação por grupo muscular ("fizeste 12/16 séries de quadricípites previstas esta semana"). Requer ≥3 semanas registadas para ser útil. Trabalho: aproveitar `volume-compute.ts` mas alimentar com `workout_sessions.entries[].actual` em vez do plano prescrito.