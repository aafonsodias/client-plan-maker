...
Phased plans (R72 C1) = source of truth = `workout_plan_days` rows. `plan_data` é snapshot derivado para PDF / legacy readers. Qualquer regen TEM de passar por `persistRegeneratedPlan` (src/server/plan.functions.ts) que apaga + reinsere todas as day rows e dá bump ao `plan_data_version`. Escrever só em `plan_data` faz o plano "voltar atrás" porque `MicrocyclePanel` tem realtime sub em `workout_plan_days` e `PlanEditorSurface` rebuild a view a partir das rows em cada load.
...
- [Audit log + adaptation engine](mem://architecture/audit-and-adaptation.md) — audit_events (append-only, trigger-enforced), screening_evaluations (immutable), logAuditEvent helper, adaptation-next-block@0.1.0 deterministic engine scaffold (Epley e1RM delta, RPE drift, adherence). No AI in the engine — AI only rewords transitionPrompt.
- [Leap-of-faith bets](mem://strategy/leap-of-faith.md) — 4 falsifiable hypotheses (trust, pricing, log-adherence, multi-block differentiation) + monthly review cadence
- [Feature MVP template](mem://principles/feature-mvp-template.md) — required one-pager before any new surface >1 day; ties feature → bet → metric → kill criteria
