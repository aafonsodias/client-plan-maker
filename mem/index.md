...
App theme (R71) = paleta terapêutica baseada em evidência. 3 modos no ThemeToggle: deep (escuro, teal-night hue 215) · sage (médio, teal-grey hue 195) · mist (claro, parchment hue 200). Acento quente = terracota suave hue 45 a ~10% de cobertura (CTAs, focus rings, chips activos). Class hooks mantidos (`:root` / `.slate` / `.light`) para back-compat — só os valores oklch mudaram em src/styles.css. FORGE amber (`--forge-*`, BrandMark, founder badge, PDF export) NÃO mudou — vive em paralelo. Status semantic colours (success emerald, warn amber, danger red) também mantêm-se via status-tone.ts.
