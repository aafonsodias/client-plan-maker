...
export function PaywallDialog({
  ...
  const subscribe = async (tier: TierId) => {
    ...
    try {
      ...
      toast.error(e?.message ?? t("paywall.checkout_failed"));
      setBusy(null);
    }
  };

  const title = t(reason === "trial_ended" ? "paywall.title_trial" : "paywall.title_quota");
  const subtitle = t(reason === "trial_ended" ? "paywall.subtitle_trial" : "paywall.subtitle_quota");
      ...
      <DialogContent className="max-w-3xl">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {TIERS.map((tier) => {
            ...
            const suffix = interval === "year" ? t("paywall.per_year") : t("paywall.per_month");
            const features = t(tier.featuresKey, { returnObjects: true }) as string[];
            return (
              <div
                key={tier.id}
                className={`relative rounded-lg border bg-card p-4 ${
                  tier.highlight ? "border-accent/60 shadow" : "border-border"
                }`}
              >
                {tier.highlight && (
                ...
                <div className="text-xs font-bold uppercase tracking-widest text-accent">{tier.name}</div>
                <div className="mt-2">
                ...
                >
                  {busy === tier.id ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : null}
                  {t("paywall.subscribe")}
                </Button>
              </div>
            );
        ...
        <div className="flex justify-between pt-2 text-xs text-muted-foreground">
          ...
          <button type="button" onClick={() => onOpenChange(false)} className="hover:text-foreground">
            {t("paywall.later")}
          </button>
        </div>
      </DialogContent>
