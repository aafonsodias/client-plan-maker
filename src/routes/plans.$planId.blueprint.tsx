...
import { Loader2, RefreshCw, ArrowRight, ArrowLeft, Plus, Trash2 } from "lucide-react";
...
function BlueprintReview() {
  ...
  async function load() {
    ...
    const { data } = await supabase
      .from("workout_plans")
      .select("title, blueprint, generation_state")
      .eq("id", planId)
      .maybeSingle();
    if (!data) {
  ...
  async function regenerate() {
    ...
    setBusy(false);
    if (!res.ok) {
      toast.error(res.error || "Blueprint failed");
      return;
    }
  ...
  if (!blueprint) {
    return (
      <div className="mx-auto max-w-4xl p-8 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Generating blueprint…</p>
      </div>
    );
  }
